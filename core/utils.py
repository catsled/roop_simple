...
def create_video(video_name, fps, output_dir):
    ...
    os.system(f'ffmpeg -framerate "{fps}" -i "{output_dir}{sep}%04d.png" -c:v libx264 -crf 7 -pix_fmt yuv420p -y "{output_dir}{sep}input.mp4"')
...
def add_audio(output_dir, target_path, video, keep_frames, output_file):
    ...
    os.system(f'ffmpeg -i "{output_dir_ff}{sep}input.mp4" -i "{output_dir_ff}{sep}{video}" -c:v copy -map 0:v:0 -map 1:a:0 -y "{save_to_ff}"')
    if not os.path.isfile(save_to):
        shutil.move(output_dir + "/input.mp4", save_to)
    if not keep_frames:
